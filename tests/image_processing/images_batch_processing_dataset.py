...
async def process_images(image_paths, tile_size, filter_type, output_dir, batch_size=5):
    """Send images in batches, collect per-batch and overall timing."""
    import base64
    ...
    batch_times = []  # wall-clock time per batch (distributed_map only)
    images_processed = 0
    ...
    for batch_idx in range(num_batches):
        ...
        batch_paths = image_paths[batch_start_pos : batch_start_pos + batch_size]
        batch_num = batch_idx + 1

        # ── Encode images for this batch ──────────────────────────
        task_inputs = []
        for img_offset, image_path in enumerate(batch_paths):
            global_idx = batch_start_pos + img_offset
        ...
        print(
            ...
            f"images {batch_start_pos + 1}-{batch_start_pos + len(batch_paths)} "
            f"({len(batch_paths)} images)"
        )

        batch_start = time.time()
        raw_results = await crowdio_map(process_image_worker, task_inputs)
        batch_time = time.time() - batch_start
        batch_times.append(batch_time)

        # ── Parse and save each result in this batch ──────────────
        for result_idx, raw in enumerate(raw_results):
            ...
            image_path = batch_paths[result_idx]
            ...
            print(
                f"    [{batch_start_pos + result_idx + 1}/{total_images}] "
                f"{os.path.basename(image_path)} -> {os.path.basename(result_path)} "
                f"({len(tiles)} tiles, avg_tile={avg_tile:.4f}s)"
            )

        images_processed += len(batch_paths)
        print(
            f"  Batch {batch_num} time: {batch_time:.2f}s "
            f"({len(batch_paths)} images, "
            f"{len(batch_paths) / batch_time:.2f} img/s)"
        )
    ...
    print(f"  Per-batch times (s)    : {', '.join(f'{t:.2f}' for t in batch_times)}")
    avg_batch = sum(batch_times) / len(batch_times) if batch_times else 0
    print(f"  Avg batch time (s)     : {avg_batch:.2f}")
    print(f"  Overall wall time (s)  : {overall_time:.2f}")
    overall_throughput = images_processed / overall_time if overall_time > 0 else 0
