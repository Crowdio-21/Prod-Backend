...
# =========================================================
# � RUNTIME CHECKPOINT INTERVAL SETTER
# =========================================================
def set_checkpoint_interval(interval: float):
    """
    Mutate the checkpoint_interval on the decorated worker at runtime.
    This updates the metadata that gets serialised & sent to workers.
    """
    monte_carlo_euler_worker.__crowdio_metadata__.checkpoint_interval = interval
    monte_carlo_euler_worker.__crowdio_config__.metadata.checkpoint_interval = interval
...
async def run_distributed_monte_carlo_euler(
    total_trials, num_workers=None, foreman_host="localhost", checkpoint_interval=5.0
):
    """
...
def print_run_result(metrics):
    ...
    ci = metrics.get("checkpoint_interval", "?")
...
def print_summary_table(all_metrics):
    ...
    print("\n")
    print_separator()
    print("  BENCHMARK SUMMARY TABLE  (checkpoint enabled)")
    print_separator()
    header = (
        f"{'Trials':>8s}  {'Workers':>7s}  {'ChkptInt':>8s}  {'Job Exec (s)':>13s}  "
...
def print_aggregate_averages(all_metrics):
    """Print averages grouped by checkpoint interval, trials, and workers."""
    from collections import defaultdict

    # --- Averages grouped by checkpoint_interval ---
    by_interval = defaultdict(list)
    for m in all_metrics:
        by_interval[m.get("checkpoint_interval", "?")].append(m)

    print("\n  AVERAGES BY CHECKPOINT INTERVAL")
    print("-" * 90)
    header = f"{'ChkptInt':>8s}  {'Avg Job Exec (s)':>17s}  {'Avg Task Time (s)':>18s}  {'Avg Throughput (trials/s)':>26s}"
    print(header)
    print("-" * len(header))
    for ci in sorted(by_interval, key=lambda x: float(x) if isinstance(x, (int, float)) else 999):
        runs = by_interval[ci]
        ...
        print(f"{str(ci):>8s}  {avg_jet:>17.3f}  {avg_att:>18.3f}  {avg_tp:>26,.2f}")
            ...
            writer.writerow(
                {
                    ...
                    "checkpoint_interval": m.get("checkpoint_interval", "?"),
...
async def run_benchmark_grid(foreman_host="localhost"):
    """
    Run the full benchmark grid with checkpoint enabled:
      Trials    : 1M, 2M, 5M  (uncomment for 10M, 50M, 100M, 1B)
      Intervals : 5.0, 2.0, 1.0, 0.5, 0.1, 0.05
      Workers   : 6 (fixed — constant workload per worker)
    ...
    total_runs = len(trial_counts) * len(intervals)
    all_metrics = []

    print_separator()
    print(f"  CHECKPOINT-ENABLED MONTE CARLO EULER BENCHMARK")
    print(f"  Checkpoint  : ON")
    print(f"  Workers     : 6 (fixed)")
    print(f"  Trials grid : {', '.join(format_trials(t) for t in trial_counts)}")
    print(f"  Intervals   : {', '.join(str(i) for i in intervals)}")
    ...
    for trials in trial_counts:
        for interval in intervals:
            ...
            print(
                ...
                f"{format_trials(trials)} trials, 6 workers, "
                f"chkpt_interval={interval}s"
            )
    ...
    if successful:
        ...
        csv_path = os.path.join(
            ...
            f"chkpt_enabled_FIFO_benchmark_checkpoint_intervals_results.csv",
        )
...
async def main():
    """
    Entry point for checkpoint-enabled benchmarking.

    Usage:
        # Step 1 — Calibrate: run 1M trials to find best checkpoint interval
        python chkpt-enabled_FIFOautomated_monte_carlo_euler_client.py --calibrate
        python chkpt-enabled_FIFOautomated_monte_carlo_euler_client.py --calibrate <foreman_host>

        # Step 2 — Full grid with a chosen interval (e.g. 1.0s)
        python chkpt-enabled_FIFOautomated_monte_carlo_euler_client.py --interval 1.0
        python chkpt-enabled_FIFOautomated_monte_carlo_euler_client.py --interval 1.0 <foreman_host>

        # Or do both: calibrate then immediately run the full grid with the best interval
        python chkpt-enabled_FIFOautomated_monte_carlo_euler_client.py --calibrate --run
    """
    foreman_host = "localhost"  
    await run_benchmark_grid(
        foreman_host=foreman_host
    )
