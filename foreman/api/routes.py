...
async def kill_task(task_id: str, db: AsyncSession = Depends(get_db)):
    """
    Signal a running task to stop.
    The worker will return 'killed' as the task result on the next loop iteration.
    Returns 202 Accepted — the kill is async; the final result arrives via the
    worker's TASK_RESULT message.
    """
    from foreman.main import ws_manager
    ...
    if task.status != "assigned":
        raise HTTPException(
            status_code=409,
            detail=f"Task {task_id} is not currently running (status={task.status})",
        )

    sent = await ws_manager.task_dispatcher.send_kill_to_worker(task_id)
    if not sent:
        raise HTTPException(
            status_code=503, detail=f"Worker for task {task_id} is not reachable"
        )

    return {"status": "kill_sent", "task_id": task_id}
