...
async def get_evaluation_metrics(
    ...
    job_ids: Optional[str] = Query(
        None, description="Comma-separated job IDs to filter"
    ),
    since_hours: Optional[int] = Query(
        None, description="Only include data from last N hours"
    ),
):
    """
...
async def get_job_evaluation(job_id: str, db: AsyncSession = Depends(get_db)):
...
async def get_load_distribution(
    db: AsyncSession = Depends(get_db), job_ids: Optional[str] = Query(None)
):
...
async def _get_job_metrics(
    db: AsyncSession, job_ids: Optional[List[str]], since: Optional[datetime]
    ...
    for job in jobs:
        ...
        metrics.append(
            JobMetricsResponse(
                ...
                completion_rate=(
                    job.completed_tasks / job.total_tasks if job.total_tasks > 0 else 0
                ),
...
async def _get_load_balancing_metrics(
    db: AsyncSession, job_ids: Optional[List[str]]
) -> LoadBalancingMetricsResponse:
    """Get load balancing metrics."""
    # Count completed assignments per worker from task_assignments
    query = (
        select(
            TaskAssignmentModel.worker_id,
            func.count(TaskAssignmentModel.id).label("task_count"),
        )
        .where(TaskAssignmentModel.status == "completed")
        .group_by(TaskAssignmentModel.worker_id)
    )

    if job_ids:
        query = query.join(
            TaskModel, TaskModel.id == TaskAssignmentModel.task_id
