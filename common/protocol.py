...
class Message:
    ...
    def __init__(
        self, msg_type: MessageType, data: Dict[str, Any], job_id: Optional[str] = None
    ):
        self.type = msg_type
        self.data = data
        self.job_id = job_id
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"type": self.type.value, "data": self.data, "job_id": self.job_id}
...
def create_submit_job_message(
    func_code: str, args_list: List[Any], job_id: str
) -> Message:
...
def create_job_accepted_message(job_id: str) -> Message:
    ...
    return Message(MessageType.JOB_ACCEPTED, {"job_id": job_id}, job_id)
...
def create_job_results_message(results: List[Any], job_id: str) -> Message:
    ...
    return Message(
        msg_type=MessageType.JOB_RESULTS, data={"results": results}, job_id=job_id
    )


def create_worker_ready_message(worker_id: str) -> Message:
    """Create a worker ready message"""
    return Message(msg_type=MessageType.WORKER_READY, data={"worker_id": worker_id})


def create_ping_message() -> Message:
    """Create a ping message"""
    return Message(msg_type=MessageType.PING, data={})


def create_pong_message() -> Message:
    """Create a pong message"""
    return Message(msg_type=MessageType.PONG, data={})
...
def create_task_checkpoint_message(
    ...
    return Message(msg_type=MessageType.TASK_CHECKPOINT, data=data, job_id=job_id)


def create_checkpoint_ack_message(
    task_id: str, job_id: str, checkpoint_id: int
) -> Message:
...
def create_device_topology_message(
    job_id: str,
    ...
    """Create a topology message that distributes graph structure to workers."""
    return Message(
        msg_type=MessageType.DEVICE_TOPOLOGY,
        data={
...
def create_topology_update_message(
    job_id: str,
    inference_graph_id: str,
    reason: str,
    nodes: List[Dict[str, Any]],
    ...
    """Create a topology update message for dynamic failure adaptation."""
    return Message(
        msg_type=MessageType.TOPOLOGY_UPDATE,
        data={
            "inference_graph_id": inference_graph_id,
            "reason": reason,
            "nodes": nodes,
...
def create_intermediate_feature_message(
    job_id: str,
    task_id: str,
    ...
    """Create a message for forwarding intermediate features between topology stages."""
    return Message(
        msg_type=MessageType.INTERMEDIATE_FEATURE,
        data={
            "task_id": task_id,
...
def create_aggregation_config_message(
    job_id: str,
    aggregation_strategy: str,
    strategy_params: Optional[Dict[str, Any]] = None,
) -> Message:
    """Create a message that configures branch merge behavior for DNN jobs."""
    return Message(
        msg_type=MessageType.AGGREGATION_CONFIG,
        data={
...
def create_fallback_decision_message(
    job_id: str,
    task_id: str,
    fallback_mode: str,
    reason: Optional[str] = None,
) -> Message:
    """Create a message instructing worker fallback behavior for degraded topology."""
    return Message(
        msg_type=MessageType.FALLBACK_DECISION,
        data={
            "task_id": task_id,
